...
public class WashineCoreCommunity implements WashineCoreCommunityIf {
	...
	@Override
	public boolean nameInJoinedCommunities(String name, String communityUid) {
		return false;
	}
	...
	public boolean joinCommunity(String uid, String invitationCode, String communityName) {
		...
		try {
			...
			if (communityUid == uid || communityUid == null || washineCommunity.alreadyAdded(communityUid, uid)) {
				return false;
			} else {
				return washineCommunity.addParticipationToGroup(communityUid, uid,
						washineCommunity.getInvitationName(invitationCode));
			}
		} catch (SQLException e) {
			return false;
		}
