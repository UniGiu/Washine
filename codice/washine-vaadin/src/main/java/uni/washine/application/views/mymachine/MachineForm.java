...
public class MachineForm extends VerticalLayout {
  ...
  public void init(String washingId) {
    ...
    if (options.getDatetime() > 0)
      dateTimeWashingPicker.setValue(
          WashineTimeUtils.unixTimestampToLocalDate(options.getDatetime()));
    if (options.getWashingAccessOpenDate() > 0)
      accessOpenDatePicker.setValue(
          WashineTimeUtils.unixTimestampToLocalDate(options.getWashingAccessOpenDate()));
    if (options.getWashingAccessCloseDate() > 0)
      accessCloseDatePicker.setValue(
          WashineTimeUtils.unixTimestampToLocalDate(options.getWashingAccessCloseDate()));

    if (options.getDurationMinutes() > 0)
      durationField.setValue((double) options.getDurationMinutes());
    if (options.getInitialLoad() > 0) initialLoadField.setValue(options.getInitialLoad());
    if (options.getMaxLoad() > 0) maxLoadField.setValue(options.getMaxLoad());
    // this one is required and can have defaut 0
    // if (options.getVisibilityTime() > 0)
    visibilityTimeField.setValue((double) options.getVisibilityTime() / (24 * 60));
  ...
  /**
   * Provides validation state of the form and updates the fields to their validity state
   *
  ...
  /**
   * Checks if the washing that is going to be updated has already collectd participants and if so
  ...
  /** Opens a confirmation dialog in order to proceed to the update of the 'dirty' washing */
